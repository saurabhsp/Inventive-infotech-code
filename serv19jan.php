...
<input type="hidden" name="discountamt" id="discountamt" value="0">
  ...
  <div class="charge-fields">
    ...
    <div class="mini">
      ...
      <input type="text" name="packing_gst" id="packing_gst" class="inp" readonly
             value="<?php echo h((string)($editingHeader['packing_gst'] ?? 0)); ?>">
    </div>
...
/* -----------------------------
 * 5. PACKING & FORWARDING (FINAL FINAL)
 * ----------------------------- */

// 1️⃣ Packing amount (Basic Total ka % ya manual)
const packingBase = computeCharge(
  getBasicTotal(),   // % always on BASIC TOTAL
  'packing_per',
  'packing_amt'
);

// 2️⃣ GST on packing amount
const packingGstPer = val('packing');
const packingGstAmt = packingGstPer > 0
  ? (packingBase * packingGstPer / 100)
  : 0;

// 3️⃣ Packing + GST (yeh packing_gst me dikhana hai)
const packingWithGST = packingBase + packingGstAmt;

// 4️⃣ Total = Basic Cost With GST + Packing (with GST)
const packingTotal =
  val('nettotal') +   // Basic Cost With GST
  packingWithGST;

// 5️⃣ SET VALUES
setv('packing_amt', packingBase);        // 223.00
setv('packing_gst', packingWithGST);     // 234.15
setv('packing_total', packingTotal);     // 26548.15

  /* -----------------------------
 * 6. TRANSPORT / COURIER (FIXED)
 * ----------------------------- */

// 1️⃣ Transport base = Packing & Forwarding TOTAL (packing + GST)
const transportBase = computeCharge(
  val('packing_gst'),   // ✅ base changed here
  'trans_per',
  'trans_amt'
);

// 2️⃣ GST on transport
const transportGstPer = val('trans');
const transportGstAmt = transportGstPer > 0
  ? (transportBase * transportGstPer / 100)
  : 0;

// 3️⃣ Transport total (amount + GST)
const transportTotal = transportBase + transportGstAmt;
setv('trans_total', transportTotal);

// 4️⃣ Total with Transport
// = Packing & Forwarding TOTAL + Transport TOTAL
const total4 =
  val('packing_total') +
  transportTotal;
...
setv('total4', total4);
    ...
    if (insuranceAmtInput > 0) {
      insuranceBaseAmt = insuranceAmtInput;
    } else if (insurancePer > 0) {
...
function hookPerAmt(perId, amtId, baseFn){
  ...
  const run = () => {
    ...
    computeTotals(); // keep totals in sync
  };
...
if(qtypeEl){
  ...
  // totals at start
  computeTotals();
  togglePerAmtLock('packing_per','packing_amt', getBasicTotal);
togglePerAmtLock('trans_per','trans_amt', getBasicTotal);
