...
$TABLE_NATURE_LOG        = 'jos_ierp_complaint_naturelog';
...
if ($resMS) {
    while ($r = $resMS->fetch_assoc()) $mechanicStatusOptions[] = $r;
    $resMS->free();
}
...
SELECT
    ...
    GROUP_CONCAT(
        DISTINCT ct.name
        ORDER BY ct.name SEPARATOR ', '
    ) AS complaint_types,
    GROUP_CONCAT(
        DISTINCT cn.name
        ORDER BY cn.name SEPARATOR ', '
    ) AS complaint_natures,
    GROUP_CONCAT(
        DISTINCT nl.complaintdesc
        ORDER BY nl.id SEPARATOR ' | '
    ) AS complaint_nature_desc,
...
LEFT JOIN {$TABLE_NATURE_LOG} nl  ON nl.saleid = c.id
LEFT JOIN {$TABLE_COMPLAINT_TYPE} ct  ON ct.id = nl.complainttype
LEFT JOIN {$TABLE_COMPLAINT_NATURE} cn  ON cn.id = nl.complaintnature
{$whereSql}
GROUP BY c.id
ORDER BY c.id DESC
...
if (isset($_GET['export']) && $_GET['export'] === 'excel') {
    ...
    header("Content-Disposition: attachment; filename=complaint_report_" . date('d-m-Y') . ".xls");
    ...
    foreach ($rows as $r) {
        ...
        echo "<tr>
            ...
            <td>" . h($r['complaint_types']) . "</td>
            <td>" . h($r['complaint_natures']) . "</td>
            <td>" . h($r['complaint_status_name']) . "</td>
            <td>{$sd}</td>
            <td>{$sch}</td>
            <td>" . h($r['mechanic_names']) . "</td>
            <td>{$md}</td>
            <td>" . h($r['mech_status_name']) . "</td>
            ...
            <div style="display:grid; grid-template-columns:repeat(6,minmax(0,1fr)); gap:12px; align-items:end;">
                ...
                <div class="field" style="grid-column:5/7">
                    ...
                    <div style="display:flex;gap:8px;justify-content:flex-end">
                        ...
                        <a href="complaint_orderlist.php" class="btn secondary" style="display:inline-block; text-align:center; min-width:120px;">Reset</a>
                        <a class="btn primary"
                            href="complaint_orderlist.php?<?php echo http_build_query(array_merge($_GET, ['export' => 'excel'])); ?>">
                            Export Excel
                        </a>
                    </div>
                </div>
        ...
        <div class="table-wrap">
            <table class="table">
                ...
                <tbody>
                    <?php if (empty($rows)): ?>
                        <tr>
                            <td colspan="14" style="text-align:center; padding:18px;">No records found.</td>
                        </tr>
                        <?php else:
                        $sr = $offset + 1;
                        foreach ($rows as $r):
                        ...
                        ?>
                            <tr>
                                ...
                                <td><?php echo (int)($r['orderno'] ?? 0); ?> <div class="muted" style="font-size:11px;"></div>
                                </td>
                                <td><?php echo h($d); ?></td>
