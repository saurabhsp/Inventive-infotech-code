...
foreach ($grid as $r) {
    ...
    if (num($r['gst'] ?? 0) > 0 || num($r['gstamt'] ?? 0) > 0) {
        $showGstCol = true;
    }
}
...
$stt = $con->prepare("
    ...
    FROM jos_erp_servicetermsconditions
    ORDER BY id ASC
");
...
$headerDiscount = num($hdr['discountamt'] ?? 0);
...
$erectionTotal = isset($hdr['commission_total']) ? num($hdr['commission_total']) : 0.0;
...
/* ============================================================
 * FINAL TABLE COLUMN CALCULATION (DYNAMIC)
 * ============================================================ */

// base columns: Sr, Particular, Qty, Rate, Amount
$totalCols =
    5
    + ($showDiscCol  ? 1 : 0)
    + ($showLoyalCol ? 1 : 0)
...
// label colspan (everything except last Amount column)
$labelColspan = $totalCols - 1;

// total row with Qty + Amount
$qtyTotalLabelColspan = $totalCols - 2;
...
foreach ($grid as $r) {
    ...
    $productName = trim($r['product_name'] ?? 'Item');
    $itemDesc    = trim($r['description'] ?? '');
    ...
    $gstAmt = num($r['gstamt'] ?? 0);

    // Final amount without GST
    $amtwithnogst = $amt - $gstAmt;
    ...
    $rowsHtml .= '<td>
    <div class="b">' . h($productName) . '</div>' .
        ($itemDesc !== ''
            ? '<div class="small" style="margin-top:3px;">' . nl2br(h($itemDesc)) . '</div>'
            : '') . '
</td>';
    ...
    $rowsHtml .= '<td class="r nowrap">' . money2($amtwithnogst) . '</td>';
...
/* ============================================================
 * LOAD COMPANY QR CODE (SUBCOMPANY WISE)
 * ============================================================ */
...
$stQr = $con->prepare("
    SELECT uploadimg 
    FROM jos_crm_qrcodeimg 
    LIMIT 1
");

$stQr->execute();
$rsQr = $stQr->get_result()->fetch_assoc();
$stQr->close();

if ($rsQr && !empty($rsQr['uploadimg'])) {

    $qrPath = __DIR__ . '/../uploads/' . $rsQr['uploadimg'];

    if (file_exists($qrPath)) {
        $qrImg = file_get_contents($qrPath);
        $qrBase64 = 'data:image/png;base64,' . base64_encode($qrImg);
    }
}
...
if (!empty($terms)) {
    ...
    foreach ($terms as $t) {
        ...
        $titleRaw   = trim($t['title'] ?? '');
        $titleLower = strtolower($titleRaw);

        $termsHtml .= '
        <div style="margin-bottom:12px;">
            <div class="b">' . h($titleRaw) . '</div>
            <div style="margin-top:4px;">' . $t['description'] . '</div>
        </div>';

        // 🔥 Add QR ONLY after COMMERCIAL section
        if (strpos($titleLower, 'commercial') !== false && !empty($qrBase64)) {
...
if ($customerName !== '') {
    $customerBlock .= '<div class="b">' . h($customerName) . '</div>';
}

/* ADDRESS */
if (!empty($cust['address'])) {
    $customerBlock .= '<div style="margin-top:4px;">' . h($cust['address']) . '</div>';
}

/* MOBILE */
if (!empty($cust['mobile'])) {
    $customerBlock .= '<div style="margin-top:4px;"><span class="b">Mobile No :</span> ' . h($cust['mobile']) . '</div>';
}

/* EMAIL */
if (!empty($cust['billemail'])) {
    $customerBlock .= '<div style="margin-top:4px;"><span class="b">E-Mail :</span> ' . h($cust['billemail']) . '</div>';
}
...
if ($showDiscCol) {
    $colgroupHtml .= '<col style="width:12%">';
}
if ($showLoyalCol) {
    $colgroupHtml .= '<col style="width:12%">';
}
...
$colgroupHtml .= '<col style="width:10%">';




if ($showDiscCol) {
    $theadHtml .= '<th>Discounted Price</th>';
}

if ($showLoyalCol) {
    $theadHtml .= '<th>Additional Loyalty</th>';
}

if ($showAddCol) {
    $theadHtml .= '<th>Additional Discount</th>';
}
...
// TOTAL (Qty + Amount)
// TOTAL (Qty + Amount) — FIXED ALIGNMENT
$totalsHtml .= '
<tr class="totline">
    <td colspan="2" class="r b">Total</td>   <!-- Sr + Particular -->
    <td class="r b nowrap ">' . h(money2($totalQty)) . '</td>  <!-- Qty -->

    <td></td>  <!-- Rate -->

';

if ($showDiscCol)  $totalsHtml .= '<td></td>'; // Discounted Price
if ($showLoyalCol) $totalsHtml .= '<td></td>'; // Loyalty
if ($showAddCol)   $totalsHtml .= '<td></td>'; // Additional Discount
if ($showGstCol)   $totalsHtml .= '<td></td>';  //GST


$totalsHtml .= '
    <td class="r b nowrap">' . h(money2($subTotal)) . '</td> <!-- Amount -->
</tr>';;
...
$totalsHtml .= '
<tr class="totline">
    <td colspan="' . $labelColspan . '" class="r b">Sub Total</td>
...
if ($headerDiscount > 0) {
    $totalsHtml .= '
<tr class="totline">
    <td colspan="' . $labelColspan . '" class="r b">Discount</td>
...
$totalsHtml .= '
<tr class="totline">
    <td colspan="' . $labelColspan . '" class="r b">Basic Cost With GST</td>
...
if ($packingTotal > 0) {
    $totalsHtml .= '
<tr class="totline">
    <td colspan="' . $labelColspan . '" class="r b">Packing Charges</td>
    <td class="r b nowrap ">' . h(money2($packingTotal)) . '</td>
</tr>';
}
...
if ($transportTotal > 0) {
    $totalsHtml .= '
<tr class="totline">
    <td colspan="' . $labelColspan . '" class="r b">Transport Charges</td>
    <td class="r b">' . h(money2($transportTotal)) . '</td>
</tr>';
}
...
if ($insuranceTotal > 0) {
    $totalsHtml .= '
<tr class="totline">
    <td colspan="' . $labelColspan . '" class="r b">Insurance Charges</td>
    <td class="r b nowrap">' . h(money2($insuranceTotal)) . '</td>
</tr>';
}
...
if ($erectionTotal > 0) {
    $totalsHtml .= '
<tr class="totline">
    <td colspan="' . $labelColspan . '" class="r b">Erection & Commissioning Charges</td>
    <td class="r b nowrap">' . h(money2($erectionTotal)) . '</td>
</tr>';
}
...
$totalsHtml .= '
<tr class="totline">
    <td colspan="' . $labelColspan . '" class="r b">Grand Total</td>
    <td class="r b nowrap">' . h(money2($grandTotal)) . '</td>
</tr>';



//echo "QR VALUE LENGTH: " . strlen($qrBase64);
//exit;
...
' . $totalsHtml . '
 ...
 <div class="pad" style="border-top:1px solid #111; page-break-inside: avoid;">
