...
$types = '';
$params = [];
//Doc type separation of outward list
$where[] = 's.doc = ?';
$types  .= 'i';
$params[] = 27;
...
SELECT
    ...
    s.tolc AS to_location,

    u.name AS created_by_name
FROM jos_ierp_gatepass s
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['export'] ?? '') === 'excel') {
  ...
  header("Content-Disposition: attachment; filename=gatepass_outward_" . date('d-m-Y') . ".xls");
...
if (
  ...
  try {
    ...
    $st = $con->prepare(
      "DELETE FROM jos_ierp_gatepass_grid WHERE billid = ?"
    );
    ...
    $st = $con->prepare(
      "DELETE FROM jos_ierp_gatepass WHERE id = ?"
    );
    $st->bind_param("i", $delId);
    $st->execute();
    $st->close();
...
<div class="master-wrap">
  ...
  <div class="headbar">
    <div>
      <h1 class="page-title">Material Gate Pass Outward List</h1>
  ...
  <div class="card" style="margin-top:12px;">
    <form method="post">
      ...
      <div style="display:grid;grid-template-columns:repeat(6,1fr);gap:10px;align-items:end">
        ...
        <div class="field">
          <label>M.GatePass Outword No.</label>
            ...
            <tr>
              ...
              <td>
                <div style="display:flex; gap:6px;">
                  ...
                  <?php if ($canEdit): ?>
                    <form method="post" action="../operations/materialgatepass_outward.php" title="Edit">
                      <input type="hidden" name="csrf_token" value="<?= h(csrf_token()) ?>">
                      <input type="hidden" name="mode" value="edit_load">
                  ...
                  <!--<form method="post" action="/operations/issuegatepass_print.php" target="_blank" title="Print">-->
                  <!--  <input type="hidden" name="csrf_token" value="<?= h(csrf_token()) ?>">-->
                  ...
                  <form method="post" action="/operations/materialgatepass_outwardprint.php" style="margin:0;" target="_blank">
