...
/* ==========================
   TABLE
========================== */
$TABLE = 'jos_erp_sale_order';
$TABLE_GRID   = 'jos_erp_saleorder_grid';
...
FROM jos_erp_sale_order s

LEFT JOIN jos_erp_saleorder_grid g 
    ON g.saleid = s.id
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['export'] ?? '') === 'excel') {
    ...
    header("Content-Disposition: attachment; filename=saleorder_products" . date('d-m-Y') . ".xls");
    ...
    foreach ($rows as $i => $r) {
        ...
        echo "<tr>
                <td>{$sr}</td>
                <td>" . fy_from_date($r['salesdate']) . "/" . h($r['saleno']) . "</td>
                <td>" . dmy($r['salesdate']) . "</td>
...
<div class="master-wrap">
    ...
    <div class="headbar">
        <div>
            <h1 class="page-title">Sales Order Product List</h1>
    ...
    <div class="card" style="margin-top:12px;">
        <form method="post">
            ...
            <div style="display:grid;grid-template-columns:repeat(6,1fr);gap:10px;align-items:end">
                ...
                <div class="field">
                    <label>From</label>
                    <input type="text" name="from" placeholder="DD-MM-YYYY" class="inp datepick" value="<?= h($from) ?>">
                </div>
                <input type="hidden" name="view_change" id="view_change" value="">

                <div class="field">
                    <label>To</label>
                    <input type="text" name="to" placeholder="DD-MM-YYYY"  class="inp datepick" value="<?= h($to) ?>">
                </div>
                ...
                <div class="field">
                    ...
                    <input type="text" name="saleno" class="inp" value="<?= h($saleno) ?>">
                </div>
                ...
                <tbody>
                    ...
                    <?php foreach ($rows as $i => $r): ?>
                        <tr>
                            ...
                            <td>
                                <strong>
                                    <?= fy_from_date($r['salesdate']) ?>/<?= h($r['saleno']) ?>
                                </strong>
                                <br>
                                <?= dmy($r['salesdate']) ?>
                            </td>
