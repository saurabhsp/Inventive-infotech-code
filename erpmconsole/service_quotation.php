...
<div class="totals-wrap" style="margin-top:18px; border-top:1px solid #e5e7eb; padding-top:14px;">
  ...
  <div class="totals-grid" style="margin-top:10px;">
    ...
    <div class="field">
      ...
      <input type="text" name="discountamt" id="discountamt" class="inp" readonly
             value="<?php echo h((string)($editingHeader['discountamt'] ?? 0)); ?>">
    </div>
  ...
  <div class="modal-dialog">
    ...
    <!-- <div class="field">
     ...
     <div style="margin-bottom:10px; position:relative;">
      <!-- this is the line of dropdown-->
      <label>Product<span class="req">*</span></label>
      <input type="text" id="rm_product" class="inp" placeholder="Select product from suggestions">
      <div id="rm_product" style="position:relative;"></div>
...
const EXISTING_TITLE = <?php echo json_encode((string)($editingHeader['title'] ?? '')); ?>;
...
function renderGrid(){
  ...
  gridRows.forEach((r, idx)=>{
    ...
    tr.innerHTML = `
      ...
      <td>${money(r.gst)}</td>
      <td class="muted">${escapeHtml(r._split||'')}</td>
...
function updateRowModalPreview(){
  ...
  let inst   = n(document.getElementById('rm_inst')?.value);   // ✅ FIXED
  ...
  let discounted = gross - (disc + inst);
...
function renderSuggest(items){
  closeSuggest();
  const host = document.getElementById('rm_product');
  ...
  suggestBox.style.boxShadow='0 10px 30px rgba(0,0,0,.12)';
  ...
  items.forEach(it => {
    ...
    d.innerHTML = `<div style="font-weight:600;">${esc(it.label)}</div>`;
...
function computeTotals(){
  ...
  // base (before header discount)
  let base = subtotal + gst;

  // header discount
  const disctype = parseInt(document.getElementById('disctype')?.value || '0',10) || 0;
  const discount = val('discount');
  let discountamt = 0;
  if (discount > 0){
    discountamt = (disctype === 1) ? (base * discount / 100) : discount;
    if (discountamt > base) discountamt = base;
  }
  let baseAfterDiscount = Math.max(0, base - discountamt);

  // write base fields
  document.getElementById('subtotal').value = money(subtotal);
  document.getElementById('gst').value = money(gst);
  document.getElementById('discountamt').value = money(discountamt);

  // nettotal/total2 = baseAfterDiscount
  document.getElementById('nettotal').value = money(baseAfterDiscount);
  document.getElementById('total2').value = money(baseAfterDiscount);

  // charges
  const packingBase = computeCharge(baseAfterDiscount, 'packing_per', 'packing_amt');
  const packingTotal = computeChargeTotal(packingBase, val('packing'));
  setv('packing_total', packingTotal);

  const transBase = computeCharge(baseAfterDiscount, 'trans_per', 'trans_amt');
  const transTotal = computeChargeTotal(transBase, val('trans'));
  setv('trans_total', transTotal);

  // total4 = baseAfterDiscount + packingTotal + transTotal
  const total4 = baseAfterDiscount + packingTotal + transTotal;
  setv('total4', total4);
  ...
  const commissionBase = computeCharge(total4, 'commission_per', 'commission_amt');
  const commissionTotal = computeChargeTotal(commissionBase, val('commission'));
  setv('commission_total', commissionTotal);

  const total5 = total4 + insuranceTotal + commissionTotal;
  setv('total5', total5);

  // tcs on total5
  const tcsPer = val('tcs');
  const tcsamt = (tcsPer>0) ? (total5 * tcsPer / 100) : 0;
  document.getElementById('tcsamt').value = money(tcsamt);

  const total3 = total5 + tcsamt;
  setv('total3', total3);
...
function baseTotalForCharges(){
  // Prefer Total (total2). If not present use subtotal.
  const t = numVal('total2');
  return t > 0 ? t : numVal('subtotal');
}
...
document.addEventListener('DOMContentLoaded', function(){
  ...
  hookPerAmt('commission_per','commission_amt',baseForInsurance);
});
...
const titleEl = document.getElementById('title_inp');
if(titleEl){
  titleEl.addEventListener('change', function(){
    loadSubjectByTitle(this.value);
  });
}
  ...
  if(EXISTING_QTYPE && EXISTING_QTYPE > 0){
    loadTitlesForQType(EXISTING_QTYPE, EXISTING_TITLE || '');
  }
