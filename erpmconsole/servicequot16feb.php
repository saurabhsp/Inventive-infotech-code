...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['quotation_form']) && $_POST['quotation_form'] === '1') {
  ...
  try {
    ...
    $tcSql = implode(',', array_map(function ($c) {
      return "`{$c}`";
    }, $useTC));
...
<style>
  ...
  @media (max-width:1200px) {
    ...
    .charge-row {
      grid-template-columns: 180px repeat(2, minmax(140px, 1fr));
    }
...
<script>
  ...
  function closeRowModal() {
    document.getElementById(PRODUCT_MODAL_ID).classList.add('hidden');
  }
  ...
  function saveTerm() {
    ...
    if (!typeId) {
      alert('Select Term Type');
      return;
    }
    if (!title) {
      alert('Master title not found for this type. Configure jos_erp_servicetermsconditions.');
      return;
    }
...
/* -----------------------------
    ...
    const total3 = basicWithGST + packingTotal + tcsAmt;
