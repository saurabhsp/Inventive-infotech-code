<?php
/* ============================================================
 * operations/issuegatepass.php  —  Material Gate Pass Issue (Core ERP, ui_autoshell)
...
<div class="card" style="margin-bottom:14px;">
  ...
  <div style="font-size:34px; font-weight:800; margin-bottom:12px;"> Material Gate Pass</div>
        ...
        <select name="to_gid" class="inp" required>
          ...
          <?php foreach($locations as $L): ?>
            <option
                value="<?= (int)$L['gid'] ?>"
                data-name="<?= h($L['nm']) ?>"
                <?= ((string)$hdr['to_gid']===(string)$L['gid'])?'selected':''; ?>
            >
                <?= h($L['nm']) ?>
            </option>
            <?php endforeach; ?>
      ...
      <table class="table" style="min-width:1180px;">
        <thead>
          <tr>
            ...
            <th style="width:170px;">2nd Qty</th>
            <th style="width:170px;">2nd Measure</th>
            <th style="width:170px;">3rd Qty</th>
            <th style="width:170px;">3rd Measure</th>
...
<div id="itemModal" style="display:none; position:fixed; z-index:9999; left:50%; top:50%; transform:translate(-50%,-50%); width:min(860px, 92vw); background:#fff; border-radius:14px; overflow:hidden;">
  ...
  <div style="padding:14px 16px;">
    ...
    <div style="margin-top:12px; display:grid; grid-template-columns: 1fr 1fr; gap:10px;">
      <div><label>2nd Unit Qty</label><input type="text" id="m_sec_qty" class="inp locked" readonly></div>
      <div><label>3rd Unit Qty</label><input type="text" id="m_third_qty" class="inp locked" readonly></div>
...
function renderGrid(){
  ...
  grid.forEach((r, idx) => {
    ...
    tr.innerHTML = `
      ...
      <td>${secMeas ? esc(secMeas) : ''}</td>
      <td>${esc(showUnitQty(r.secondconversion, r.thirdunit))}</td>
      <td>${thirdMeas ? esc(thirdMeas) : ''}</td>
...
function hideModal(){ back.style.display='none'; modal.style.display='none'; closeSuggest(); }
...
function openModal(idx, r){
  ...
  document.getElementById('m_unit').value = r ? (r.unit || '') : '';
...
function renderSuggest(items){
  closeSuggest();
  const host = document.getElementById('m_suggest');
...
document.getElementById('m_product').addEventListener('input', function(){
  ...
  searchTimer = setTimeout(async () => {
    try{
      const j = await postForm({ ajax:'product_search', q:q });
      if (j && j.ok) renderSuggest(j.items || []);
    }catch(e){ closeSuggest(); }
  }, 180);
});

document.addEventListener('click', function(e){
  const p = document.getElementById('m_product');
...
document.querySelector('[name="to_gid"]').addEventListener('change', function(){
  const opt = this.options[this.selectedIndex];
  document.getElementById('to_name').value = opt.getAttribute('data-name') || '';
});
