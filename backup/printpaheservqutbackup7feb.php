...
foreach ($grid as $r) {
    ...
    $qty   = num($r['qty'] ?? 0);
    $rate  = num($r['rate'] ?? 0);
   $disc50  = num($r['discount'] ?? 0);
$loyalty = num($r['addloyality_per'] ?? 0);
$disc3   = num($r['adddiscount_per'] ?? 0);
   ...
   if ($showDiscountCols) {
    $rowsHtml .= '<td class="r">'.($disc50 > 0 ? h(money2($rate * $disc50 / 100)) : '-').'</td>';
    $rowsHtml .= '<td class="r">'.($loyalty > 0 ? h(money2($rate * $loyalty / 100)) : '-').'</td>';
    $rowsHtml .= '<td class="r">'.($disc3 > 0 ? h(money2($rate * $disc3 / 100)) : '-').'</td>';
...
<tr>
    ...
    <th>Discounted<br>Price<br>Rate Rs.<br>Each</th>
    <th>Additional<br>Loyalty<br>Benefits<br>Rs.<br>Each</th>
    <th>Additional<br>Discount<br>Rs.<br>Each</th>
    <th>Amount</th>
</tr>
</thead>
