                ...
                if ($dup) $err = 'User already exists with this email or mobile.';
                    ...
                    if ($id > 0) {
                        ...
                        if ($pass !== '') {
                            ...
                            $st = $con->prepare("UPDATE $TABLE SET name=?, email=?, mobile_no=?, status=?, role_id=?, password_hash=? WHERE id=?");
                            $st->bind_param('sssissi', $name, $email, $mobile, $status, $role_id, $hash, $id);
                        } else {
                            $st = $con->prepare("UPDATE $TABLE SET name=?, email=?, mobile_no=?, status=?, role_id=? WHERE id=?");
                            $st->bind_param('sssiii', $name, $email, $mobile, $status, $role_id, $id);
                        }
                        ...
                        // new user
                        $hash = password_hash($pass, PASSWORD_DEFAULT);
                        $st = $con->prepare("INSERT INTO $TABLE (name,email,mobile_no,password_hash,status,role_id) VALUES (?,?,?,?,?,?)");
                        $st->bind_param('ssssii', $name, $email, $mobile, $hash, $status, $role_id);
                        if ($st->execute()) {
                            $new_id = $st->insert_id;
                            $st->close();
                            $st2 = $con->prepare("INSERT INTO $MAP_TABLE (user_id, role_id) VALUES (?,?)");
...
$type = '';
if ($q !== '') {
    $where .= " AND (u.name LIKE ? OR u.email LIKE ? OR u.mobile_no LIKE ?)";
    $like = "%$q%";
    $bind[] = $like;
    $bind[] = $like;
    $bind[] = $like;
