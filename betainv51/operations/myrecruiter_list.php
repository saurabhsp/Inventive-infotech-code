...
.master-wrap .card .table-wrap { width: 100%; overflow-x: hidden; }
.master-wrap .card .table-wrap .table { width: 100%; max-width: 100%; min-width: 0 !important; table-layout: fixed; }
.master-wrap .card .table-wrap .table th,
.master-wrap .card .table-wrap .table td { word-wrap: break-word; word-break: break-word; }
...
document.addEventListener('DOMContentLoaded', function () {
  if (window.flatpickr) {
    flatpickr(".js-date-ddmmyyyy", { dateFormat: "d-m-Y", allowInput: true });
  }
});
</script>
...
function get_str($key,$default=''){ return isset($_GET[$key]) ? trim((string)$s=$_GET[$key]) : $default; } // keep as-is
...
if ($rs = mysqli_query($con, "
    ...
    ORDER BY plan_name
")) {
  while ($r = mysqli_fetch_assoc($rs)) { $subscription_plan_opts[] = $r; }
}
...
if ($city_id!==''){ $where_common[]="u.city_id LIKE CONCAT('%',?,'%')"; $types_common.='s'; $params_common[]=$city_id; }
...
if ($created_from){ $where_common[]="DATE(u.created_at)>=?"; $types_common.='s'; $params_common[]=$created_from; }
if ($created_to){   $where_common[]="DATE(u.created_at)<=?"; $types_common.='s'; $params_common[]=$created_to; }
...
switch($sort){
  ...
  case 'name_asc':  $order = " ORDER BY COALESCE(NULLIF(rp.organization_name,''), NULLIF(rp.contact_person_name,''), u.mobile_no) ASC"; break;
  case 'name_desc': $order = " ORDER BY COALESCE(NULLIF(rp.organization_name,''), NULLIF(rp.contact_person_name,''), u.mobile_no) DESC"; break;
...
while($row = $res->fetch_assoc()):
  ...
  if ($row['last_start_date'] || $row['last_end_date']) {
    ...
    if ($row['last_end_date'] && strtotime($row['last_end_date']) >= time()) {
      $sub_status = 'Active';  $sub_status_class = 'badge success';
    } else {
      $sub_status = 'Expired'; $sub_status_class = 'badge warn';
    }
