  ...
  <div class="master-wrap">
    ...
    ?>
      <div class="card" style="padding:20px">
        ...
        <?php
        /* ==========================================
   EMBED APPLICATIONS TABLE (Same Page)
   ========================================== */
              ...
              <tbody>
                ...
                ?>
                  <tr>
                    <td><?= $sr++ ?></td>
                    <td><?= (int)$r['id'] ?></td>
                    <td><?= h($r['candidate_name'] ?: ('User #' . $r['userid'])) ?></td>
                    <td>
                      <?php
                      $contact = [];
                      if (!empty($r['mobile_no'])) $contact[] = h($r['mobile_no']);
                    ...
                    <td><?= h(fmt_date($r['application_date'])) ?></td>
                    <td><?= h((string)$r['status_id']) ?></td>
                    <td><?= h(safe_date_label($r['interview_date_time'])) ?></td>
                    <td>
...
if ($state !== '') {
  $where[] = "w.state LIKE CONCAT('%',?,'%')";
  $types  .= 's';
  $params[] = $state;
}

if ($city !== '') {
  $where[] = "w.city_id LIKE CONCAT('%',?,'%')";
  $types  .= 's';
  $params[] = $city;
}

if ($locality !== '') {
  $where[] = "w.locality_id LIKE CONCAT('%',?,'%')";
  $types  .= 's';
  $params[] = $locality;
}
...
if ($status_in === 'active') {
  $where[] = "w.job_status_id = 1";
} elseif ($status_in === 'inactive') {
  $where[] = "w.job_status_id = 0";
}
