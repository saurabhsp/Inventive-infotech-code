...
$page_title = 'Users – Jobseeker Verification List';
...
$logged_admin_role_id = (int)($me['role_id'] ?? 0);
...
if ($view !== 'all') {
    $view = 'last50'; // force safe default
} // last50|all
$page = max(1, get_int('page', 1));
// $per_page = ($view === 'all') ? 1000 : 50;
if ($view === 'all') {
    $per_page = 200; // or 100 max
} else {
    $per_page = 50;
}
...
SELECT
  ...
  u.referred_by, u.active_plan_id, u.status_id, u.created_at, vs.name AS verification_name,
  u.verfied_status,
  u.verified_by,
  u.verified_at,

  cp.candidate_name, cp.profile_photo, cp.gender_id, g.name AS gender_name,
...
" . $sql_base . $sql_where . $order . " LIMIT $per_page OFFSET $offset";
...
<body>
    ...
    <script>
        ...
        document.addEventListener("click", function(e) {
            if (!e.target.closest("#jobSearch") && !e.target.closest("#jobSuggestions")) {
                closeDropdown();
            }
        });
