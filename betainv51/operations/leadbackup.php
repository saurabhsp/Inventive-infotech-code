...
function fmt_date($dt){ return $dt ? date('d-m-Y', strtotime($dt)) : '—'; }
function fmt_dt($dt){ return $dt ? date('d-m-Y h:i A', strtotime($dt)) : '—'; }
...
function current_script_paths(): array {
  ...
  $v[] = $script;
  $v[] = $req;
  $v[] = preg_replace('#^/beta#','', $script);
  $v[] = preg_replace('#^/beta#','', $req);
  return array_values(array_unique(array_filter($v, fn($p)=> (string)$p !== '')));
}
...
$page_title = $menu['menu_name'] ?: 'Lead Entry';
...
if (!user_can('view',$MENU_ID,$con)) {
  ...
  <div style="max-width:740px;margin:24px auto;padding:14px;border:1px solid rgba(148,163,184,.15);border-radius:14px;background:rgba(2,6,23,.35)">
    <h3 style="margin:0 0 10px"><?=h($page_title)?></h3>
    <div class="badge off">You are not authorized to view this content.</div>
  </div>
  <?php
  exit;
}
...
if ($_SERVER['REQUEST_METHOD']==='POST') {
  ...
  if (!verify_csrf($_POST['csrf'] ?? null)) {
    ...
    if ($isModal) {
      ...
      if ($lead_id<=0) { header('Content-Type: application/json'); echo json_encode(['ok'=>false,'msg'=>'Invalid lead id.']); exit; }
      if ($to_status<=0 || !isset($statuses[$to_status])) { header('Content-Type: application/json'); echo json_encode(['ok'=>false,'msg'=>'Invalid status.']); exit; }
      if ($remark==='') { header('Content-Type: application/json'); echo json_encode(['ok'=>false,'msg'=>'Remark is required.']); exit; }
      ...
      $st->close();
      if (!$lead) { header('Content-Type: application/json'); echo json_encode(['ok'=>false,'msg'=>'Lead not found.']); exit; }
      ...
      if ($to_code==='FOLLOW_UP') {
        if ($followup==='') { header('Content-Type: application/json'); echo json_encode(['ok'=>false,'msg'=>'Follow-up Date/Time is required.']); exit; }
        $followup_db = parse_followup_to_db($followup);
        if (!$followup_db) { header('Content-Type: application/json'); echo json_encode(['ok'=>false,'msg'=>'Invalid follow-up date/time (use dd-mm-yyyy hh:mm AM/PM).']); exit; }
      } elseif ($to_code==='ON_BOARDED') {
        if ($plan_id==='0' || $plan_id==='') { header('Content-Type: application/json'); echo json_encode(['ok'=>false,'msg'=>'Please select On-boarded Plan.']); exit; }
      ...
      if (!$ok) { header('Content-Type: application/json'); echo json_encode(['ok'=>false,'msg'=>'Update failed.']); exit; }
    ...
    if (isset($_POST['save'])) {
      ...
      } elseif (!in_array($profile_type,[1,2],true)) {
...
if ($ptype===1 || $ptype===2) { $where.=" AND l.profile_type=?"; $bind[]=$ptype; $types.='i'; }
if ($stFilter>0) { $where.=" AND l.status_id=?"; $bind[]=$stFilter; $types.='i'; }
if ($srcFilter>0) { $where.=" AND l.source_id=?"; $bind[]=$srcFilter; $types.='i'; }
if ($assFilter>0) { $where.=" AND l.assigned_to=?"; $bind[]=$assFilter; $types.='i'; }
...
.pac-label .k{ color:#9ca3af; font-size:12px; }
.pac-label .v{ font-weight:600; margin-top:2px; color:#e5e7eb; }
@media (max-width:900px){ .pac-labelgrid{ grid-template-columns:1fr; } }
...
.flatpickr-months,.flatpickr-weekdays,.flatpickr-days,.flatpickr-time{
  background:#0b1220 !important;
  opacity:1 !important;
}
.flatpickr-weekday,.flatpickr-day{ color:#e5e7eb !important; }
.flatpickr-day.today{ border-color:rgba(96,165,250,.9) !important; }
.flatpickr-day.selected,.flatpickr-day.startRange,.flatpickr-day.endRange{
  background:rgba(34,197,94,.85) !important;
  border-color:rgba(34,197,94,.85) !important;
  color:#04110a !important;
}
.flatpickr-day:hover{ background:rgba(148,163,184,.18) !important; }
.flatpickr-time input,.flatpickr-time .flatpickr-am-pm{
  color:#e5e7eb !important;
  background:#0b1220 !important;
}
.flatpickr-current-month .flatpickr-monthDropdown-months,
.flatpickr-current-month input.cur-year{ color:#e5e7eb !important; }
...
<?php if ($mode==='denied_form'): ?>
  <div class="pac-panel" style="max-width:820px">
    <div class="pac-head">
...
<td><?=h(fmt_dt($r['updated_at'] ?? null))?></td>
  ...
  <script>
   ...
   document.getElementById('m_plan_id').innerHTML = buildPlanOptions(pt, row.onboarded_plan_id || "0");
      ...
      if (window.flatpickr) {
        try { if (_modalFP) _modalFP.destroy(); } catch(e) {}
...
<?php else: /* form */ ?>
    ...
    <form method="post" class="pac-grid3">
      ...
      <div>
        ...
        <select name="source_id" class="inp">
          ...
          <?php $sv=(int)$val('source_id',0); foreach($sources as $sid=>$nm): ?>
      ...
      <div>
        ...
        <select name="assigned_to" class="inp">
          ...
          <?php $av=(int)$val('assigned_to',0); foreach($adminUsers as $uid=>$nm): ?>
  ...
  (function(){
    ...
    function setHide(el, hide){ if(!el) return; el.classList.toggle('hide', !!hide); }
