...
<div class="master-wrap">
  ...
  <div class="card">
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['verify_user_id'])) {
      ...
      $status = isset($_POST['verfied_status']) ? (int)$_POST['verfied_status'] : 0;
    ...
    // -1 = any, 0 = not verified, 1 = verified
...
";
    ...
    /* verified filter */
    if ($verified_filter == 0 || $verified_filter == 1) {
      $where[] = "u.verfied_status=?";
      $types .= 'i';
...
SELECT
  ...
  u.referred_by, u.active_plan_id, u.status_id, u.created_at, u.verfied_status,u.verified_by,u.verified_at,
...
" . $sql_base . $sql_where . $order . " " . ($view === 'all' ? "" : " LIMIT $per_page OFFSET $offset");
    ...
    <form method="get" class="toolbar" style="gap:10px;flex-wrap:wrap">
      ...
      <select class="inp" name="verified_filter" title="Verification Status">
        <option value="-1" <?= $verified_filter == -1 ? 'selected' : '' ?>>Verification: Any</option>
