...
if (($_SERVER['REQUEST_METHOD'] ?? '') === 'POST' && isset($_POST['ajax'])) {
    ...
    if ($ajax === 'product_search') {
        ...
        $b2 = null;
        if ($codeCol !== '') {
            $where = "(`$nameCol` LIKE ? OR `$codeCol` LIKE ?)";
            $types = "ss";
            $b2 = $like;
        }
        ...
        if ($types === 'ss') $st->bind_param($types, $b1, $b2);
        else $st->bind_param($types, $b1);
        $st->execute();
        $rs = $st->get_result();
...
if ($mode === 'edit_load') {
    ...
    if ($edit_id <= 0) {
        ...
        if (!$H) {
            ...
            if ($hdr['supplier_id'] > 0) {
                $st = $con->prepare(
                    "SELECT name FROM jos_ierp_msupermaster WHERE id=? LIMIT 1"
                );
                $st->bind_param('i', $hdr['supplier_id']);
                $st->execute();
                $r = $st->get_result()->fetch_assoc();
                $st->close();
...
if ($mode === 'delete') {
    ...
    if ($did <= 0) {
        flash_set('err', 'Invalid delete id');
        $mode = 'form';
    } else {
    ...
    <form method="post" autocomplete="off" id="mainForm">
        ...
        <div class="table-wrap" style="margin-top:10px; overflow:auto;">
            <table class="table" style="min-width:1180px;">
                ...
                <tbody id="gridBody">
                    <tr class="emptyRow">
                        <td colspan="9" style="text-align:center; opacity:.7;">No items added yet.</td>
                    </tr>
                </tbody>
            </table>
        </div>
...
<script>
    ...
    (function() {
        ...
        if (window.flatpickr) init();
            ...
            loadJs('https://cdn.jsdelivr.net/npm/flatpickr', init);
        }
    })();
    ...
    let grid = [];
    try {
        grid = JSON.parse(document.getElementById('rows_json').value || '[]') || [];
    } catch (e) {
        grid = [];
    }

    function setJson() {
        document.getElementById('rows_json').value = JSON.stringify(grid || []);
    }
    ...
    function calcMeasure(baseW, baseH, qty) {
        const W = parseFloat(baseW || '0'),
            H = parseFloat(baseH || '0'),
            Q = parseFloat(qty || '0');
    ...
    document.getElementById('m_save').addEventListener('click', function() {
        ...
        if (!product_id) {
            alert('Select product from suggestions.');
            return;
        }
        if (!(qty > 0)) {
