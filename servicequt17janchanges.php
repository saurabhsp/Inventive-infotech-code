...
const transBase   = computeCharge(getBasicTotal(), 'trans_per', 'trans_amt');
   ...
   * 8. COMMISSION
   * ----------------------------- */
const commissionBase = computeCharge(getBasicTotal(), 'commission_per', 'commission_amt');
  const commissionTotal = computeChargeTotal(commissionBase, val('commission'));
  setv('commission_total', commissionTotal);

const total5Raw = total4 + insuranceTotal;
const total5 = total5Raw;
setv('total5', total5);

/* -----------------------------
 * 9. TCS (ONLY on Packing + Basic)
 * ----------------------------- */
const tcsPer = val('tcs');

// Base = Basic Cost with GST + Packing Total ONLY
const tcsBase = val('nettotal') + val('packing_total');

const tcsAmt = (tcsPer > 0) ? (tcsBase * tcsPer / 100) : 0;
setv('tcsamt', tcsAmt);

// ✅ Total with TCS = BASIC + PACKING (+ TCS)
const totalWithTcs = tcsBase + tcsAmt;
setv('total3', totalWithTcs);

/* -----------------------------
 * 10. GRAND TOTAL (ALL CHARGES)
 * ----------------------------- */
