...
$walkinQuery = "SELECT j.id, j.slug, cjp.name AS job_position, j.company_name, j.city_id AS city, 
                sfrom.salaryrange AS salary_from, sto.salaryrange AS salary_to, 
                jt.name AS job_type, ws.shift_name AS work_shift, rp.company_logo
                ...
                WHERE j.job_status_id = ?
                ORDER BY RAND()
                LIMIT 3";

$stmtW = $con->prepare($walkinQuery);
$stmtW->bind_param('i', $job_status_id);
$stmtW->execute();


$resW = $stmtW->get_result();
while ($row = $resW->fetch_assoc()) {
    $row['company_logo'] = !empty($row['company_logo'])
        ? "webservices/" . $row['company_logo']
...
$vacancyQuery = "SELECT j.id, j.slug, cjp.name AS job_position, j.company_name, j.city_id AS city, 
                 sfrom.salaryrange AS salary_from, sto.salaryrange AS salary_to, 
                 rp.company_logo
...
$stmtV->bind_param('i', $job_status_id);
$stmtV->execute();
