...
if ($application_id > 0) {
    ...
    $sql = "SELECT 
                ...
                g.name AS gender,
                s.name AS status,
                CONCAT(cp.locality_id, ', ', cp.city_id) AS location,

                CASE 
                    WHEN a.job_listing_type = 1 THEN jp1.name
                    WHEN a.job_listing_type = 2 THEN jp2.name
                    ELSE ''
                END AS jobname

            FROM jos_app_applications a

            LEFT JOIN jos_app_users u ON a.userid = u.id
            LEFT JOIN jos_app_candidate_profile cp ON u.profile_id = cp.id
            LEFT JOIN jos_crm_gender g ON cp.gender_id = g.id
            LEFT JOIN jos_app_applicationstatus s ON a.status_id = s.id

            LEFT JOIN jos_app_walkininterviews jw 
                ON (a.job_listing_type = 1 AND a.job_id = jw.id)

            LEFT JOIN jos_app_jobvacancies jv 
                ON (a.job_listing_type = 2 AND a.job_id = jv.id)

            LEFT JOIN jos_crm_jobpost jp1 ON jw.job_position_id = jp1.id
            LEFT JOIN jos_crm_jobpost jp2 ON jv.job_position_id = jp2.id
    ...
    if ($result && mysqli_num_rows($result) > 0) {
        ...
        if (!empty($application['birthdate']) && $application['birthdate'] != '0000-00-00') {
            $dob = new DateTime($application['birthdate']);
            $today = new DateTime();
            $application['age'] = $today->diff($dob)->y;
        }

        // PROFILE PHOTO
        $application['profile_photo'] = !empty($application['profile_photo'])
            ? DOMAIN_URL . ltrim($application['profile_photo'], '/')
            : DOMAIN_URL . 'webservices/uploads/nophoto.jpg';

        // TEMP KYC
        $application['kycstatus'] = true;
    }
}
...
<head>
    ...
    <title>Application Detail | Pacific iConnect</title>
...
<body>
    ...
    <div class="mobile-header">
        ...
        <span class="mobile-header-title">Application </span>
    </div>
            ...
            <div class="app-card">
                <div class="app-card-top">
                    <div class="app-user-info">
                        <div class="avatar-lg">
                            <img src="<?= $application['profile_photo'] ?>"
                        ...
                        <div class="user-details">
                            ...
                            <span class="job-title">
                                Applied for: <b><?= $application['jobname'] ?></b>
                            </span>
                        </div>
                    </div>
                ...
                <div class="app-info-grid">
                    <div class="info-col">
                        ...
                        <span class="info-val">
                            <i class="fas fa-user"></i> <?= $application['gender'] ?>
                        </span>
                    </div>
                    ...
                    <div class="info-col">
                        ...
                        <span class="info-val">
                            <?= $application['age'] ? $application['age'] . " Yrs" : "-" ?>
                        </span>
                    </div>
                    ...
                    <div class="info-col">
                        ...
                        <span class="info-val">
                            <i class="fas fa-phone-alt"></i> <?= $application['mobile_no'] ?>
                        </span>
                    </div>
                </div>
                ...
                <div class="app-card-bottom">
                    <div class="applied-date">
                        Applied on: <?= date('d-m-Y h:i A', strtotime($application['application_date'])) ?>
                    </div>

                    <div class="action-buttons">

                        <a href="tel:<?= $application['mobile_no'] ?>" class="btn-action">
                            <i class="fas fa-phone"></i> Call
                        </a>

                        <a href="https://wa.me/91<?= $application['mobile_no'] ?>"
                            target="_blank"
                            class="btn-action">
                        ...
                        <form action="candidate_profile.php" method="POST">
                            <input type="hidden" name="candidate_id" value="<?= $application['userid'] ?>">
                            <input type="hidden" name="application_id" value="<?= $application['id'] ?>">
                            <input type="hidden" name="job_cp_id" value="<?= $application['job_id'] ?>">
                        ...
                        <button class="btn-action"
                            onclick="openInterviewModal('interviewModal', <?= $application['id'] ?>)">
                            <i class="fas fa-users"></i> Schedule Interview
                        </button>

                        <button class="btn-primary"
                            onclick="openStatusModal('statusModal', <?= $application['id'] ?>)">
                            Update Status
                        </button>
