...
if (!empty($planName)) {
    ...
    if (!empty($plans['data'])) {
        foreach ($plans['data'] as $plan) {
            ...
            if (strtolower($plan['plan_name']) == strtolower($planName)) {
...
if ($profile_type_id == 1) {
    ...
    $gstresult = json_decode($responsethree, true);

    if (!empty($gstresult) && $gstresult['status'] == true) {
        $gstamt = $gstresult['data']['gst'] ?? 0;
        $gstno = $gstresult['data']['gstno'] ?? '';
    } else {
        $apierror = $gstresult['message'] ?? "Failed to fetch GST";
    }
}
            ...
            <aside class="sidebar">
                <div class="sub-card">
                    ...
                    <form method="POST">
                        ...
                        <!-- ✅ GST FIELD ONLY FOR PROFILE TYPE 2 -->
                        <?php if ($profile_type_id == 1) { ?>
                            <div class="gst-row">

                                <label class="label-bold">GST Number (Optional)</label>

                                <input type="text" name="gst_no" class="gstno-input"
                                    placeholder="Enter GST Number"
                                    value="<?php echo htmlspecialchars($gstno); ?>">
