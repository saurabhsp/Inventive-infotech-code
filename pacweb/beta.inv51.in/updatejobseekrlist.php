        ...
        <form method="post">
            ...
            <div class="form-grid">
                ...
                <div class="form-group full">
                    ...
                    <!-- hidden input -->
                    <input type="hidden" name="skills" id="skillsInput" value="<?= h($data['skills']) ?>">

                    <div class="multi-select">
                        <div class="select-box" onclick="toggleSkillsDropdown()">
                            <span id="selectedSkillsText" style="color:#94a3b8;">Select Skills</span>
                        </div>

                        <div class="checkbox-container" id="skillsDropdown">

                            <?php
                            $skills_arr = !empty($data['skills']) ? explode(',', $data['skills']) : [];

                            foreach ($skills_list as $skill):
                                $skill_name = $skill['title'] ?? '';
                                $skill_id = $skill['id'] ?? '';
                                $checked = in_array($skill_id, $skills_arr) ? 'checked' : '';
                            ?>
                                <label class="checkbox-item">
                                    <input type="checkbox"
                                        value="<?= $skill_id ?>"
                                        data-name="<?= h($skill_name) ?>"
                                        <?= $checked ?>
...
<script>
    function toggleSkillsDropdown() {
        let box = document.getElementById("skillsDropdown");
    ...
    function updateSkills() {
        let checked = document.querySelectorAll("#skillsDropdown input:checked");

        let values = [];
        let labels = [];

        checked.forEach(cb => {
            values.push(cb.value); // ✅ ID save
            labels.push(cb.getAttribute("data-name")); // ✅ NAME show
        });

        document.getElementById("skillsInput").value = values.join(",");

        let textBox = document.getElementById("selectedSkillsText");

        if (labels.length === 0) {
            textBox.innerText = "Select Skills";
            textBox.style.color = "#94a3b8";
        } else if (labels.length <= 2) {
            textBox.innerText = labels.join(", "); // ✅ names show
            textBox.style.color = "#e2e8f0";
        } else {
            textBox.innerText = labels.length + " skills selected";
            textBox.style.color = "#e2e8f0";
        }
    }
