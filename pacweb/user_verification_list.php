...
";
        ...
        $sql = "
SELECT
  u.id, u.mobile_no, u.profile_type_id, u.profile_id, u.city_id, u.address,vs.name AS verification_name,
...
" . $sql_base . $sql_where . $order . " " . ($view === 'all' ? "" : " LIMIT $per_page OFFSET $offset");
        ...
        <div class="cards-row">
            ...
            <?php foreach ($status_cards as $s): ?>
                ...
                $cnt_q = $con->prepare("SELECT COUNT(*) FROM jos_app_users WHERE verfied_status=?");
                $cnt_q->bind_param("i", $st);
                $cnt_q->execute();
                $cnt_q->bind_result($cnt);
                $cnt_q->fetch();
                $cnt_q->close();
