...
$types = '';
$params = [];
//Doc type separation of inward list
$where[] = 's.doc = ?';
$types  .= 'i';
$params[] = 29;
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['export'] ?? '') === 'excel') {
    ...
    header("Content-Disposition: attachment; filename=delivery_challan_" . date('d-m-Y') . ".xls");
...
if (
    ...
    try {
        ...
        $st = $con->prepare(
            "DELETE FROM jos_ierp_deliverychallan_grid WHERE billid = ?"
        );
        ...
        $st = $con->prepare(
            "DELETE FROM jos_ierp_deliverychallan WHERE id = ?"
        );
        $st->bind_param("i", $delId);
        $st->execute();
        $st->close();
        ...
        $_SESSION['ok'] = 'Delivery Challan deleted successfully';
    } catch (Throwable $e) {
...
<div class="master-wrap">
    ...
    <div class="headbar">
        <div>
            <h1 class="page-title">Delivery Challan List</h1>
                        ...
                        <tr>
                            ...
                            <td>
                                <div style="display:flex; gap:6px;">
                                    ...
                                    <?php if ($canEdit): ?>
                                        <form method="post" action="../operations/deliverychallanform.php" title="Edit">
